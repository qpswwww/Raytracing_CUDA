...
void GetImage(int nx,int ny, char* save_dir) {
	FILE *stream1;
...
int main() {
	GetImage(200,200,"E://code//Raytracing_CUDA//output.ppm");
	/*FILE *stream1;
	freopen_s(&stream1, "E://code//Raytracing_CUDA//output.ppm", "w", stdout);
	if (stream1 == NULL) {
		exit(9);
	}
	size_t FB_size = 3 * nx*ny * sizeof(float); //the size of a frame buffer
